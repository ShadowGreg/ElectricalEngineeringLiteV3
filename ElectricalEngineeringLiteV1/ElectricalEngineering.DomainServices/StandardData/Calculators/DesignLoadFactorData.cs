namespace ElectricalEngineering.DomainServices.StandardData.Calculators;
...
internal class DesignLoadFactorData {
    ...
    public double GetData(int equivalentNumberOfElectricalReceivers, double busUtilizationFactor) {
        ...
        if (desiredInternalKey == 0)
            desiredInternalKey = internalKeys[^1];

        double temp = _data[desiredForeignKey][desiredInternalKey];
        return temp;
    }
}